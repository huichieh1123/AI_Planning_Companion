...
import 'package:ai_planning_companion/core/services/index.dart';
import 'package:ai_planning_companion/core/models/index.dart';
import 'package:ai_planning_companion/features/home/presentation/widgets/trip_card.dart';
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void initState() {
    super.initState();
    _tripsFuture = context.read<TripService>().getMyTrips();
  }

  Future<void> _refreshTrips() async {
    setState(() {
      _tripsFuture = context.read<TripService>().getMyTrips(forceRefresh: true);
    });
  }
              ...
              FutureBuilder<List<Trip>>(
                ...
                builder: (context, snapshot) {
                  if (snapshot.connectionState == ConnectionState.waiting) {
                    ...
                    return Center(child: Text('加載失敗: ${snapshot.error}', style: const TextStyle(color: AppColors.error)));
                  } else if (!snapshot.hasData || snapshot.data!.isEmpty) {
                    return const Center(
                      child: Padding(
                        padding: EdgeInsets.all(32.0),
                        child: Text('還沒有任何行程\n點擊上方按鈕發起新行程！', textAlign: TextAlign.center, style: TextStyle(color: AppColors.textSecondary)),
                      )
                    );
                  }
